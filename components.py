...
def display_sidebar():
    """
    サイドバーに利用目的のラジオボタンと説明・入力例を表示
    """
    st.sidebar.title("利用目的を選択してください")

    # 利用目的の選択肢
    purposes = {
        "社内文書検索": {
            "description": "入力内容と関連性が高い社内文書のありかを検索できます。",
            "example": "【入力例】\n社員の育成方針に関するMTGの議事録"
        },
        "社内問い合わせ": {
            "description": "質問・要望に対して、社内文書の情報をもとに回答を得られます。",
            "example": "【入力例】\n人事部に所属している従業員情報を一覧化して"
        }
    }

    # ラジオボタンで利用目的を選択
    selected_purpose = st.sidebar.radio("利用目的を選択してください", list(purposes.keys()))
    
    # 選択した目的を強調表示
    st.sidebar.markdown(f"### 選択中: {selected_purpose}")

    # すべての利用目的の説明と入力例を常時表示
    for purpose, details in purposes.items():
        st.sidebar.markdown(f"### {purpose}")
        st.sidebar.info(details["description"])
        st.sidebar.code(details["example"], language=None)
